...
for error_range in error_range_list:
    ...
    initial_local_list=trace_information_array[0]['initial_local_list']
    ...
    for i in tqdm(range(len(BS_index_list))):
        ...
        initial_local=initial_local_list[i]
        ...
        inferred_local_list.append(inferred_local)
        error=np.linalg.norm(inferred_local-true_local[0:2])
        error_list.append(error)
    print('avg_error',sum(error_list)/len(error_list))

    inferred_local = np.array(inferred_local_list)
    np.save('inferred_local_errorrange'+str(error_range)+'.npy',inferred_local)
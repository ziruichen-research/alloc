...
optimizer = torch.optim.Adam(model.fcmodel.parameters(), lr=learning_rate)
...
for epoch in range(epochs):
    ...
    for kk, test_loader in enumerate(test_loader_list):
        if kk in used_index:
            ...
            rmse_finetuning_list.append(avg_rmse)
            rmse_finetuning_array=np.array(rmse_finetuning_list)
            np.save('error_finetuning_array',rmse_finetuning_array)
    ...
    for kk, (train_loader, dataset_idx) in enumerate(used_train_loader_list):
        for i, (data, local) in enumerate(train_loader):
            ...
            with torch.no_grad():
                ...
                ans = model.convmodel(data)
                ans = ans.reshape([batch_size, -1])
            local_pred=model.fcmodel[dataset_idx](ans)

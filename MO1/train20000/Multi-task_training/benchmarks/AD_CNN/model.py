...
class LocNet(nn.Module):
    def __init__(self,num_ant,num_car,hidden_size,num_datasets=5):
        ...
        self.fcmodel=nn.ModuleList([nn.Sequential(
    ...
    def forward(self,data,dataset_idx=0):
        batch_size=data.shape[0]
        data=data.permute([0,3,1,2])
        ans=self.convmodel(data)
        ans=ans.reshape([batch_size,-1])
        ans=self.fcmodel[dataset_idx](ans)
        return ans

...
path='/home/chenzirui/Desktop/home_mnt/32ant*32car_3.5GHz_40MHz_O1B_R501-1400_V1_24.4.12/train40000'
...
for epoch in range(epochs):
    for i, (data,local) in enumerate(train_loader):
        ...
        train_loc = local.unsqueeze(1)
        train_loc = train_loc.repeat(1, len(index), 1)
        weight = torch.arange(len(index)) + 1
        sum = torch.sum(weight)
        weight = weight / sum * len(index)
